...
author: "ESM 206 Fall 2020 + Will Geiken"
...
# I did not receive a warning about one piece being dropped. However, "Mundi" was dropped from Ki-Adi-Mundi's name as it was after the second dash. Poor guy can't catch a break. First he's gunned down on Mygeeto, now we're cutting off the last part of his name...
```
...
sw_skin_sep <- starwars %>% # Start from starwars data
  separate(col = skin_color, # Separate the `skin_color` column
           ...
           extra = "merge", # If additional spaces, merge all into second column
           remove = FALSE) # And keep the original column
...
mpg_model_class <- mpg %>% # Start with mpg
  unite(col = model_class, # Create a new column called 'model_class'
        ...
        sep = "-", # Put a "-" between the pieces
        remove = FALSE # And keep the original columns
        ) 
...
class(some_dates_converted) # Check the class, it returns "Date"
...
```{r}
# Your code here: 
gasol_hookshots <- lakers %>% #create a new data set "gasol_hookshots"
  filter(player == "Pau Gasol" & type == "hook") %>% #filter by player and shot type.
  mutate(date = ymd(date)) %>% #update the date column so it's in "Date" class
  mutate(month_pg_hook = month(date)) %>%  #add a new column for month
  count(month_pg_hook)  #add up the hookshot attempts in each month, which are now stored in the gasol_hookshots subset.
...
```{r}
# Your code here: 
storms_danger <- storms %>% #create a new subset "storms_danger"
  mutate(danger = case_when( #add a column labelled "danger" that reads
    status == "hurricane" ~ "high", #"high" when the status is "hurricane"
    TRUE ~ "moderate")) #or "moderate" for any other status
