...
# I did not receive a warning about one piece being dropped. However, "Mundi" was dropped from Ki-Adi-Mundi's name as it was after the second dash.
```
...
mpg_model_class <- mpg %>% # Start with mpg
  unite(col = model_class, # Create a new column called 'model_class'
        ...
        remove = FALSE # And *do not* remove the original columns
        ) 
...
gasol_hookshots <- lakers %>% 
  ...
  mutate(month_pg_hook = month(date)) %>% 
  count(month_pg_hook)
